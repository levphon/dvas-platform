#!/bin/bash
JAVA_HOME="/app/soft/jdk1.8.0_121"
ENV=$1
if [ -z "$ENV" ]; then
    echo "ERROR:PLEASE SPEC ENV ARGS,SUCH AS dev,test,beta,release(General Availability)...  "
    echo "app exit"
    exit 1
fi
...
SERVER_NAME=`sed '/spring.application.name/!d;s/.*=//' classes/bootstrap.properties | tr -d '\r'`
SERVER_PORT=`sed '/server.port/!d;s/.*=//' classes/bootstrap.properties | tr -d '\r'`
#LOGS_FILE=`sed '/logging.path/!d;s/.*=//' classes/bootstrap.properties | tr -d '\r'`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.glsx.vasp.BusinessApplication env=$ENV --spring.profiles.active=$ENV > $STDOUT_FILE 2>&1 &
