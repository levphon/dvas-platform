...
SERVER_NAME=`sed '/server.name/!d;s/.*=//' classes/bootstrap.properties | tr -d '\r'`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.glsx.vasp.DvaspApplication env=$ENV --spring.profiles.active=$ENV > $STDOUT_FILE 2>&1 &
